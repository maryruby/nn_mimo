...
def main():
    ...
    ber = bit_error_rate(output, sess, test_feed_dict, Y_test_matrix, treshold)
    logger.info('Model final BER: %.5f' % ber)
    rer = row_error_rate(output, sess, test_feed_dict, Y_test_matrix, treshold)
...
def bit_error_rate(nn_out_layer, session, feed_dict, Y_matrix, treshold=0.5):
